...
export async function generateCMAWorkbook(analysis, ratiosByYear, cmaInputs = {}, dataFlags = {}) {
  ...
  // Abridged financials notice — inserted between data rows and disclaimer
  let discOffset = 0
  if (dataFlags.abridged_financials) {
    const warnRow = covRows.length + 4
    const amberFill  = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFF3CD' } }
    const amberBorder = {
    ...
    const warnLabelCell = cov.getCell(warnRow, 1)
    warnLabelCell.value = '⚠  DATA NOTICE — ABRIDGED FILING'
    warnLabelCell.font  = { name: 'Arial', size: 10, bold: true, color: { argb: 'FF92400E' } }
    warnLabelCell.fill  = amberFill
    warnLabelCell.border = amberBorder
    warnLabelCell.alignment = { vertical: 'top', wrapText: true }
    const warnNoteCell = cov.getCell(warnRow, 2)
    warnNoteCell.value = dataFlags.abridged_note
    warnNoteCell.font  = { name: 'Arial', size: 9, italic: true, color: { argb: 'FF78350F' } }
    warnNoteCell.fill  = amberFill
    warnNoteCell.border = amberBorder
    warnNoteCell.alignment = { vertical: 'top', wrapText: true }
    cov.getRow(warnRow).height = 72
    discOffset = 1
  }
  const disc = covRows.length + 4 + discOffset
  ...
  if (mpbfNotApplicable) {
    ...
    const amberFill4 = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFF3CD' } }
    if (dataFlags.abridged_financials) {
      applyCell(f4, naRow, 1, '⚠  WORKING CAPITAL DATA UNAVAILABLE — ABRIDGED FILING', bbFont, amberFill4, { vertical: 'middle' })
      years.forEach((_, i) => applyCell(f4, naRow, i + 2, 'N/A', { name: 'Arial', size: 10, italic: true, color: { argb: 'FF999999' } }, amberFill4, { horizontal: 'center' }))
      applyCell(f4, naRow + 1, 1, dataFlags.abridged_note, { name: 'Arial', size: 8, italic: true, color: { argb: 'FF78350F' } }, amberFill4, { wrapText: true, vertical: 'top' })
    } else {
